...
import { JCAMPImporter } from '../data/jcamp-importer.js';
import { FilterManager } from './filter-manager.js';
import { LoadingOverlay, Toast, ErrorHandler, MicroInteractions } from './ui-utilities.js';
import { handleSubstanceChange } from './substance-selection.js';

/**
 * Enable a button by id, if present.
 */
export function enableExportButton(id) {
...
function selectedSubstanceSlug() {
    const name = dom.substanceSelect.options[dom.substanceSelect.selectedIndex].text;
...
function selectedSubstanceRecord() {
    const id = dom.substanceSelect.value;
    if (!id || !ctx.libraryData) return null;
    return ctx.libraryData.find(item => item.id === id) || null;
}
...
function buildPeakTable() {
    return ctx.currentPeaks.map(peak => {
        const midi = ctx.midiOutput
            ? ctx.midiOutput.frequencyToMIDI(peak.audioFreq)
        ...
        return {
            ...
            functional_group: ctx.frequencyMapper.getFunctionalGroup(peak.wavenumber)
        };
    });
}
 ...
 */
export function handleExportPeaksCSV() {
 ...
 */
export function handleExportPeaksJSON() {
    if (!ctx.currentPeaks || ctx.currentPeaks.length === 0) {
        Toast.warning('Please select a substance first');
        return;
    }
    ...
    try {
        ...
        const payload = {
            ...
            mapping: {
                irRange_cm1: [ctx.frequencyMapper.IR_MIN, ctx.frequencyMapper.IR_MAX],
                audioRange_hz: [ctx.frequencyMapper.AUDIO_MIN, ctx.frequencyMapper.AUDIO_MAX],
                scale: 'logarithmic'
            },
            peakDetection: {
                threshold: ctx.frequencyMapper.DEFAULT_THRESHOLD,
                maxPeaks: ctx.frequencyMapper.DEFAULT_MAX_PEAKS,
                minProminence: ctx.frequencyMapper.MIN_PROMINENCE,
 ...
 */
export function handleExportSpectrumCSV() {
    if (!ctx.currentSpectrum || ctx.currentSpectrum.length === 0) {
        Toast.warning('Please select a substance first');
        return;
    }
    ...
    try {
        const csv = [
            ...
            ...ctx.currentSpectrum.map(p => `${Number(p.wavenumber.toFixed(2))},${Number(p.transmittance.toFixed(2))}`)
        ].join('\n');
...
export async function handleCSVImport(e) {
    ...
    try {
        ...
        ctx.libraryData.push(data);
        FilterManager.setLibrary(ctx.libraryData);

        dom.substanceSelect.value = ctx.libraryData.length - 1;
 ...
 * @param {Event} e
 */
export async function handleJCAMPImport(e) {
    const file = e.target.files[0];
    if (!file) return;
    ...
    try {
        ...
        data.id = ctx.libraryData.length.toString();
        ctx.libraryData.push(data);
        FilterManager.setLibrary(ctx.libraryData);

        dom.substanceSelect.value = data.id;
        handleSubstanceChange();
 ...
 */
export async function withExportButton(id, busyLabel, idleLabel, run, errorPrefix) {
    const btn = document.getElementById(id);
    try {
        if (btn) {
            btn.disabled = true;
 ...
 */
export async function handleExportWAV() {
    if (!ctx.currentPeaks || ctx.currentPeaks.length === 0) {
        Toast.warning('Please select a substance first');
        return;
    }
    ...
    await withExportButton(
        ...
        async () => {
            ...
            await ctx.audioEngine.exportWAV(ctx.currentPeaks, duration, filename);
 ...
 */
export async function handleExportMP3() {
    if (!ctx.currentPeaks || ctx.currentPeaks.length === 0) {
        Toast.warning('Please select a substance first');
        return;
    }
    ...
    await withExportButton(
        ...
        async () => {
            ...
            await ctx.audioEngine.exportMP3(ctx.currentPeaks, duration, filename, 128);
