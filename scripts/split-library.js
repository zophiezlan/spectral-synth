...
const LIBRARY_FILE = path.join(ROOT, 'data', 'ftir-library.json');
...
function splitLibrary() {
    ...
    console.log('\nReading data/ftir-library.json...');
    const rawLibrary = fs.readFileSync(LIBRARY_FILE, 'utf8');
    ...
    const outputDir = path.join(ROOT, 'dist', 'library');
    ...
    const originalSize = fs.statSync(LIBRARY_FILE).size;
